...
export default defineComponent({
  ...
  setup() {
    ...
    const init = () => {
      ...
      renderer.setSize(window.innerWidth * 0.9, height);
      mainContainerDiv.appendChild(renderer.domElement);

      camera = new PerspectiveCamera(
        25,
        (window.innerWidth * 0.9) / height,
        0.1,
        2000
      );
      ...
      loader.load("assets/marker_anim_web.glb", function (gltf) {
        model = gltf.scene;
        group.add(model);
        ...
        light.position.set(15, -15, 5); //default; light shining from top
        light.intensity = 3;
        group.add(light);
